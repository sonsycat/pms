...
import com.zjry.system.domain.SysUserRole;
import com.zjry.system.mapper.SysPostMapper;
...
public class SysUserServiceImpl implements ISysUserService
{
    ...
    public List<SysUser> selectUserList(SysUser user)
    {
    	...
    	if(loginUser.getCurrDeptId() != null) {
    		user.setDeptId(loginUser.getCurrDeptId());
    	} else {
     ...
     */
    @Override
    public String selectUserRoleIdGroup(String userName)
    {
        List<SysRole> list = roleMapper.selectRolesByUserName(userName);
        StringBuffer idsStr = new StringBuffer();
        for (SysRole role : list)
	...
	public boolean changeRole(Long roleId) {
		LoginUser loginUser = SecurityUtils.getLoginUser();
        // 判断当前用户是否拥有该角色
        String roleIds = selectUserRoleIdGroup(loginUser.getUsername());
        if(StringUtils.isNotEmpty(roleIds) && roleIds.indexOf(roleId + ",") != -1) {
        	loginUser.setCurrRoleId(roleId);
	...
	@Override
	public void changeDept(Long deptId) {
		LoginUser loginUser = SecurityUtils.getLoginUser();
    	loginUser.setCurrDeptId(deptId);
    	setLoginUser(loginUser);
	}
	...
	private void setLoginUser(LoginUser loginUser) {
		if (StringUtils.isNotNull(loginUser) && StringUtils.isNotEmpty(loginUser.getToken())) {
	        ...
	        // 根据uuid将loginUser缓存
	        String userKey = zjRyConfig.getName() + ":" + Constants.LOGIN_TOKEN_KEY + loginUser.getToken();
