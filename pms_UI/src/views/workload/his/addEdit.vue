...
import { listHis, getHis, delHis, addHis, updateHis, exportHis } from "@/api/workload/his";
  ...
  watch: {
    id: {
      handler(val) {
        ...
        if (val) {
          this.id = val;
          this.title = "工作量-HIS收费项目管理-编辑";
          this.getDetailInfo();
        } else {
          this.id = "";
          this.title = "工作量-HIS收费项目管理-新增";
        }
      },
      deep: true,
      immediate: true
