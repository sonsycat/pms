    ...
    <el-dialog :title="title" :visible.sync="openDialog" width="70%" custom-class="el-button--primary" append-to-body :close-on-click-modal="false" @close="handleDialogClose">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="指标编码" prop="indexCode">
          <el-input v-model="form.indexCode" placeholder="请输入指标编码"  maxlength="20" show-word-limit  />
        </el-form-item>
        <el-form-item label="分值" prop="scorecardValue">
          <el-input v-model="form.scorecardValue" placeholder="请输入分值"  maxlength="10,2" show-word-limit  />
        </el-form-item>
...
export default {
  ...
  watch: {
    id: {
      handler(val) {
        ...
        if (val) {
          this.id = val;
          this.title = "平衡记分卡管理-编辑";
          this.getDetailInfo();
        } else {
          this.id = "";
          this.title = "平衡记分卡管理-新增";
        }
      },
      deep: true,
      immediate: true
  ...
  methods: {
  	getDetailInfo() {
      getScorecard(this.id).then(response => {
