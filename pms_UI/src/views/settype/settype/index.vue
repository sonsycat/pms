<template>
  ...
  <div ref="search">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['settype:settype:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['settype:settype:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['settype:settype:remove']"
        >删除</el-button>
      </el-col>
    ...
    >
      ...
      <el-table-column sortable label="类别名称" align="left" prop="bscName" show-overflow-tooltip />
      <el-table-column sortable label="类别编码" align="left" prop="bscCode" />
  ...
  methods: {
    ...
    getList() {
      ...
      listSettype(this.queryParams).then(response => {
        this.loading = false;
        this.settypeList = this.handleTree(
          response.rows,
          "bscCode",
          "parentCode"
        );
    ...
    handleDelete(row) {
      const ids = row.id || this.ids;
      const bscCode = row.bscCode || this.bscCodes;
      this.$confirm('是否确认删除指标集分类编号为"' + bscCode + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delSettype(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有指标集分类数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
          this.exportLoading = true;
          return exportSettype(queryParams);
        }).then(response => {
