...
<script>
  import { listPlus, getPlus, delPlus, addPlus, updatePlus, exportPlus } from '@/api/demo/plus'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.loading = true
        listPlus(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
      ...
      handleUpdate(row) {
        ...
        getPlus(inviteId).then(response => {
        ...
        this.$refs['form'].validate(valid => {
          if (valid) {
            ...
            if (this.form.inviteId != null) {
              updatePlus(this.form).then(response => {
                this.msgSuccess('修改成功')
                this.open = false
                this.getList()
              })
            } else {
              addPlus(this.form).then(response => {
