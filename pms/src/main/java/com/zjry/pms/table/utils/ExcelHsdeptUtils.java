...
import com.zjry.pms.table.domain.PmsTableHsdeptRes;
...
public class ExcelHsdeptUtils {
    public static Workbook buildData(Workbook wb, List<PmsTableHsdeptRes> list) throws IOException {
        // 创建sheet页
        Sheet sheetName = wb.createSheet("sheetName");
        sheetName.setColumnWidth(0,5000);
        // 填充表头
        Row row = sheetName.createRow(0);
        row.createCell(0).setCellValue("核算科目");
        ...
        row.createCell(2).setCellValue("实际收入");
        row.getCell(2).setCellStyle( createStyles(wb).get("header"));
        row.createCell(5).setCellValue("计价收入");
        ...
        row2.createCell(3).setCellValue("门诊");
        row2.getCell(3).setCellStyle( createStyles(wb).get("header"));
        row2.createCell(4).setCellValue("住院");
        ...
        row2.createCell(6).setCellValue("门诊");
        row2.getCell(6).setCellStyle( createStyles(wb).get("header"));
        row2.createCell(7).setCellValue("住院");
        ...
        if(null !=list && list.size()>0){
            for (int i = 0; i < list.size(); i++) {
                PmsTableHsdeptRes excelDto = list.get(i);
                row = sheetName.createRow(i + 2);
                row.createCell(0).setCellValue(excelDto.getClassName());
                row.getCell(0).setCellStyle( createStyles(wb).get("data"));
                row.createCell(1).setCellValue(excelDto.getAccountincomes());
                row.getCell(1).setCellStyle( createStyles(wb).get("data"));
                row.createCell(2).setCellValue(excelDto.getFacaccountincomes());
                row.getCell(2).setCellStyle( createStyles(wb).get("data"));
                row.createCell(3).setCellValue(excelDto.getArmaccountincomes());
                row.getCell(3).setCellStyle( createStyles(wb).get("data"));
                row.createCell(4).setCellValue(excelDto.getHrealincomes());
                row.getCell(4).setCellStyle( createStyles(wb).get("data"));
                row.createCell(5).setCellValue(excelDto.getHvalationincomes());
                row.getCell(5).setCellStyle( createStyles(wb).get("data"));
                row.createCell(6).setCellValue(excelDto.getCrealincomes());
                row.getCell(6).setCellStyle( createStyles(wb).get("data"));
                row.createCell(7).setCellValue(excelDto.getCvalationincomes());
